...
if (isset($_POST['email'])) {
    ...
    if ($result->num_rows == 0) {
        ...
        echo "<script>alert('This email is not registered. Please check and try again.'); window.location.href='forgot_password.html';</script>";
        exit();
    }
    ...
    try {
        ...
        $mail->send();
        echo "<script>alert('OTP sent to your email. Please check.'); window.location.href='reset_password.php';</script>";
    } catch (Exception $e) {
        echo "<script>alert('Mailer Error: {$mail->ErrorInfo}'); window.location.href='forgot_password.html';</script>";
    }
} else {
